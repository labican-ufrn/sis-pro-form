<template>
	<div class="entidades">
		<header class="entidades__cabecalho">
			<h1 class="entidades__titulo">Orientandos</h1>
			<p class="entidades__descricao">
				Todos os orientandos já cadastrados no sistema estão listados aqui. Eles podem
				...
				orientandos ou atualizar os já existentes.
			</p>
		</header>
		...
		<div class="entidades__conteudo">
			<ListaPesquisadoresResponsaveis
				:dados="filtrados"
				rotulo-nome="Nome completo do orientando"
				rotulo-editar="Editar orientando"
				rotulo-excluir="Excluir orientando"
				@editar="editar"
				@excluir="excluir"
			/>
		</div>
...
const filtrados = computed(() => {
	...
	if (!termo) return store.orientandos

	return store.orientandos.filter(item =>
...
const irParaNovo = () => {
	router.push({ name: 'Novo Orientando' })
}

const editar = (id: number) => {
	router.push({ name: 'Editar Orientando', params: { id: String(id) } })
}
